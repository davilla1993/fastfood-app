...
<head>
    ...
    <title>Pyramid Fastfood | Update Item</title>
...
<body>
	  <div class="container">
		...
		<h1 style="text-align:center; font-weight:bold">Update Item</h1>
				...
				<input type="hidden" name="iditem" value="${item.iditem}"/>
					<div class="form-group col-md-12">
						...
						<select name="category" class="form-control" id="category">
							<c:forEach items="${listCategory}" var="category">
								<c:if test="${category.idcategory eq item.category.idcategory}">
									<option value="${category.idcategory}" selected="selected">${category.name}</option>	
								</c:if>
								
								<c:if test="${category.idcategory ne item.category.idcategory}">
					...
					<div class="form-group col-md-12">
						<label for="email">Price</label> 
						 <input type="number" step="0.01" class="form-control" id="price" name="price" value="${item.price}" />
					</div>
			
					<div class="form-group col-md-12">
						   <input type="file" name="itemImage" id="image" />
						   <img id="thumbnail" alt="Image Preview" src="data:image/jpg;base64,${item.base64Image}" style="width:10%; margin-top:1px"/>
					</div>
	...
	$(document).ready(function(){
		...
		$("#itemForm").validate({
			...
			messages: {
				...
				image : "You must choose an image of the item"
